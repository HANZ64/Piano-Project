...
let createKey = (type, note, octave) => {
  ...
  keyBtn.className = type === 'white' ? 'piano__key piano__key--white' : 'piano__key piano__key--black';
  keyBtn.dataset.letterNote = type === 'white' ? note+octave : note + '#' + octave;
  keyBtn.dataset.letterNoteFileName = type === 'white' ? note+octave : note + 's' + octave;
  ...
  keyBtn.addEventListener('mouseup', () => keyBtn.classList.remove('piano__key--playing'));
  return keyBtn;
}
